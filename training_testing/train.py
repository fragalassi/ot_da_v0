...
'''
Fichier pour la version vanilla de Unet 3D
Pas utilisé par isensee.
...
config["image_shape"] = (176,176,176)  # This determines what shape the images will be cropped/resampled to. TO TEST

config["patch_shape"] = None  # switch to None to train on the whole image
#config["patch_shape"] = (64,64,64)  # switch to None to train on the whole image

config["labels"] = (1, 2, 4)  # the label numbers on the input image
config["labels"] = (1)  # TO TEST

#config["n_labels"] = len(config["labels"])
config["n_labels"] = 1

config["all_modalities"] = ["t1","flair"]
config["all_modalities"] = ["T1-norm-include","FLAIR-norm-include"]

#config["all_modalities"] = ["T1-norm-include","FLAIR-norm-include","ATLAS-wm_mask-reg-include","ATLAS-gm_mask-reg-include","ATLAS-csf_mask-reg-include", "mask-int-include"]; 

#config["all_modalities"] = ["T1_preprocessed_res","FLAIR_preprocessed_res"]
...
config["n_epochs"] = 400  # cutoff the training after this many epochs
...
config["data_file"] = os.path.abspath("miccai16_data.h5")
config["model_file"] = os.path.abspath("miccai16_isensee_2017_model.h5")
config["training_file"] = os.path.abspath("miccai16_isensee_training_ids.pkl")
config["validation_file"] = os.path.abspath("miccai16_isensee_validation_ids.pkl")
...
class Train:
    ...
    def fetch_training_data_files(self, return_subject_ids=False):
        ...
        subject_ids = list()
        for subject_dir in glob.glob(os.path.join(os.path.dirname(__file__), "data_miccai16", "training_FG", "*")):
