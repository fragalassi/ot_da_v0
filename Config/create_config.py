...
def create_conf_with_l(rev, batch_size=[], initial_lr = [], loss_funcs = [], depth=[],
                       ...
                       distance = [], OT_depth = [], jdot_beta=[], load_model = [], split_list = [], intensity_ceil = [], skip_blank = [],
                       n_repeat = 1):
    '''
    ...
    if not all(len(l) == the_len for l in it):
        ...
        df = pd.DataFrame(np.array([batch_size*n_repeat, initial_lr*n_repeat, loss_funcs*n_repeat,
                                    ...
                                    OT_depth*n_repeat, jdot_beta*n_repeat, load_model*n_repeat, split_list*n_repeat, intensity_ceil*n_repeat,
                                    skip_blank*n_repeat], dtype=object)).T
